...
public class SimpleEnemyAIBehavior : MonoBehaviour
{
    ...
    public float visualAngle = 45.0f; //Angle of vision
    ...
    void AIFunctionality()
    {
        ...
        updateAnimationManager(AnimationCharacterManager.CharacterStateEnum.Normal, 0.0f);
    ...
    bool TargetIsInSight()
    {
        ...
        // if the target is very close we ignore the vision angle
        if (distance < visualRadius * 0.2) // 20% of visual radious
        {
            return true;
        }

        // Now check if target is in vision angle
        if (angle < visualAngle)
        { // I am looking mostly towards target
            return true;
        } else
        {
            return false;
    ...
    void MoveTowards(Vector3 direction)
    {
        ...
        updateAnimationManager(AnimationCharacterManager.CharacterStateEnum.Normal, mustRun ? 1.0f : 0.5f);
    }
    ...
    void MonitorGravity()
    {
        ...
        {
            characterController.Move(direction * Time.deltaTime);
            updateAnimationManager(AnimationCharacterManager.CharacterStateEnum.Normal, 0.5f);
        }
    }
